 ...
 * and POSTs each chunk to the ingest-thought endpoint.
 *
 * Usage:
...
function stripQuotedReplies(text: string): string {
  ...
  for (const line of lines) {
    ...
    if (/^From:.*@/.test(line.trim()) && cleaned.length > 0) break;
...
function chunkText(text: string): Chunk[] {
  ...
  for (const para of paragraphs) {
    ...
    if (currentWords + paraWords > CHUNK_TARGET_MAX && currentWords >= CHUNK_TARGET_MIN) {
      // Emit current chunk
      chunks.push({
...
async function main() {
  ...
  for (const ref of messageRefs) {
    ...
    if (isChunked) {
      // Step 1: Ingest parent document (full body, type: document)
      const parentContent = buildEmailContent(email.body, email.from, email.subject, email.date);
      const parentResult = await ingestThought(parentContent, "gmail");
      ...
      if (!parentResult.ok || !parentResult.id) {
        ...
        // Step 2: Ingest each chunk linked to the parent
        for (const chunk of email.chunks) {
          ...
          const result = await ingestThought(chunkContent, "gmail", parentResult.id, chunk.index);

          if (result.ok) {
            ingested++;
            console.log(
              `   -> Chunk ${chunk.index + 1}: ${result.type} — ${(result.topics || []).join(", ")}`,
            );
          } else {
            errors++;
            allChunksOk = false;
      ...
      const result = await ingestThought(content, "gmail");
  ...
  console.log(`  Chunked emails:   ${chunked} (split into ${chunked > 0 ? totalChunks - (processed - chunked) : 0} chunks)`);
  console.log(`  Total thoughts:   ${totalChunks} (${processed - chunked} single + ${chunked > 0 ? totalChunks - (processed - chunked) : 0} from chunking)`);
  if (!args.dryRun) {
    console.log(`  Ingested:         ${ingested}`);
    console.log(`  Errors:           ${errors}`);
  }

  const estimatedCost = (totalWords / 750) * 0.00002 + (totalChunks * 0.00015);
