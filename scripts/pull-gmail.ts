...
async function main() {
  ...
  for (const ref of messageRefs) {
    ...
    // Build label metadata for this email
    const gmailLabels = email.labels
      .map((id) => labelMap.get(id) || id)
      .filter((name) => !name.startsWith("CATEGORY_")); // drop Gmail's internal category labels
    ...
    if (isChunked) {
      ...
      const parentResult = await ingestThought(parentContent, "gmail", undefined, undefined, emailMeta);
      ...
      if (!parentResult.ok || !parentResult.id) {
        ...
        for (const chunk of email.chunks) {
          ...
          const result = await ingestThought(chunkContent, "gmail", parentResult.id, chunk.index, emailMeta);
      ...
      const result = await ingestThought(content, "gmail", undefined, undefined, emailMeta);
